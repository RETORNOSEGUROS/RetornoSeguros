/*******************  Firebase (com proteção contra app duplicado)  *******************/
(function initFirebaseOnce() {
  // Se você carrega mais de um JS na mesma página, este guard evita "app/duplicate-app".
  if (!window.__RETORNO_FIREBASE_APP__) {
    try {
      // se já existir um app (por qualquer outro script), só reusa
      window.__RETORNO_FIREBASE_APP__ = firebase.app();
    } catch (e) {
      // senão, inicializa agora
      window.__RETORNO_FIREBASE_APP__ = firebase.initializeApp(firebaseConfig);
    }
  }
})();

// Sempre use essas referências únicas
const app  = window.__RETORNO_FIREBASE_APP__;
...
async function carregarNegociosRBAC(){
  ...
  // 5) Filtro RBAC por agência para gerente-chefe
