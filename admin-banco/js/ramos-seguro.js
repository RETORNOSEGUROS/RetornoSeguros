...
function salvarRamo() {
  const id = document.getElementById("ramoId").value.trim() || document.getElementById("campo").value.trim();
  ...
  db.collection("ramos-seguro").doc(id).set({ nomeExibicao, campo, ordem }).then(() => {
    alert("Ramo salvo com sucesso.");
    limparFormulario();
    carregarRamos();
  });
}
